...
export class Peer {
    ...
    constructor(peerN:number, orgN:number, port:number) {
        const org = "org" + orgN.toFixed(0)
        const Org = "Org" + orgN.toFixed(0)
        const peer = "peer" + peerN.toFixed(0)
        const User = "User" + (peerN+1).toFixed(0)
        ...
        this.peerEndpoint = envOrDefault('PEER_ENDPOINT', 'localhost:' + port.toFixed(0));
